...
export default function AdminRawDataPage() {
  ...
  async function assignRawStudents(ids: string[], agentName: string) {
    for (const id of ids) {
      await fetch("/api/raw-students", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id, assignedAgent: agentName }),
      });
    }
    refetch();
  }
