...
import { CheckCircle2, Database, PhoneCall, MessageCircle, Sparkles } from "lucide-react";
...
export default function AgentRawDataPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/leads?source=raw&sort=newest");
        if (cancelled) return;
        if (res.ok) {
          ...
          const mapped: Lead[] = (data.leads ?? []).map((r: Record<string, unknown>) => ({
            ...
            intentReasons: [],
            leadType: (r.leadType as LeadType) ?? "raw",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Raw student data imported by admin and assigned to you. Same controls as New Leads — call, update status, set follow-ups.</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Assigned to you", value: assigned, icon: Database, color: "bg-blue-100 text-blue-700" },
          { label: "Calls connected", value: called, icon: PhoneCall, color: "bg-green-100 text-green-700" },
          { label: "Pending calls", value: pending, icon: CheckCircle2, color: "bg-amber-100 text-amber-700" },
        ].map((m) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {leads.map((lead) => (
                <tr key={lead.id} className="group hover:bg-slate-50">
                  ...
                  <td className="p-3.5">
                    <Badge className="bg-slate-200 text-slate-700">Imported Student</Badge>
                  </td>
                  <td className="p-3.5">
                  ...
                  <td className="p-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <a href={waLink(lead.phone, `Hi ${lead.name}! This is Orion Education. You have a college preference on file. Can we help?`)} target="_blank" rel="noreferrer">
