...
export function RawStudentEditModal({ student, open, onOpenChange }: { student: RawStudentRecord | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      await fetch("/api/raw-students", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: studentId, ...draft }),
      });
      ...
      <DialogContent className="max-w-2xl bg-white">
        ...
        <div className="max-h-[70vh] space-y-4 overflow-y-auto pr-1">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5"><Label className="text-xs text-slate-600">Assigned agent</Label><select value={draft.assignedAgent ?? ""} onChange={(event) => set("assignedAgent", event.target.value || undefined)} className="h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-sm"><option value="">Unassigned</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></div>
          ...
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-4"><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button variant="gold" onClick={save} disabled={saving}>{saving ? "Saving..." : "Save changes"}</Button></div>
        </div>
      </DialogContent>
    </Dialog>
  );
